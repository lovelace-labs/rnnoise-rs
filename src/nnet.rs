...
/// (float-weights path). Always carries a bias for the RNNoise model.
...
impl LinearLayer {
    /// `compute_linear` (float path): `out = W·in + bias (+ diag·in for GRU)`.
    fn compute(&self, out: &mut [f32], input: &[f32]) {
        let n = self.nb_outputs;
        let m = self.nb_inputs;
        match &self.weights_idx {
            Some(idx) => sparse_sgemv8x4(&mut out[..n], &self.float_weights, idx, n, input),
            None => {
                // Dense: out[i] = sum_j W[j*N + i] * in[j].
                //
                // Accumulate as a sequence of SAXPYs (j outer, i inner). This
                // streams the weight matrix sequentially (cache-friendly,
                // auto-vectorizable) while preserving the exact per-`out[i]`
                // accumulation order (j ascending), so results are bit-identical
                // to the strided form and to the C reference.
                let w = &self.float_weights;
                let out = &mut out[..n];
                for v in out.iter_mut() {
                    *v = 0.0;
                }
...
pub(crate) fn compute_gru(input_w: &LinearLayer, recurrent_w: &LinearLayer, state: &mut [f32], input: &[f32]) {
...
pub(crate) fn compute_rnn(
    ...
    compute_conv1d(&model.conv1, &mut tmp, &mut rnn.conv1_state, input, CONV1_IN, Activation::Tanh);
    compute_conv1d(&model.conv2, &mut cat, &mut rnn.conv2_state, &tmp, CONV2_IN, Activation::Tanh);

    compute_gru(&model.gru1_input, &model.gru1_recurrent, &mut rnn.gru1_state, &cat[..CONV2_OUT]);
    compute_gru(&model.gru2_input, &model.gru2_recurrent, &mut rnn.gru2_state, &rnn.gru1_state);
    compute_gru(&model.gru3_input, &model.gru3_recurrent, &mut rnn.gru3_state, &rnn.gru2_state);
